#include "Proccess_BOMBA.h"
#include "..\..\..\Common\SQL.h"
#include "..\..\..\Util\metadataseparator.h"
#include "..\..\..\Util\LettertoNumber.h"
#include <sstream>

void tasks::buildings::proccess_BOMBA_DE_AGUA(int CityID,std::string Field)
{
	sql::Driver *driver;
	sql::Connection *conn;
	sql::Statement *stmt;
	...
	int CurrentWater = atoi(Stored.TagValue.c_str());
	CurrentWater += sacarAgua;
	std::stringstream nuevo;
	nuevo << "Type=BOMBA_DE_AGUA;Size=" << atoi(size.TagValue.c_str()) << ";Remaining=" << RemainingWater - sacarAgua << ";MJStored=" << CurrentMJint << ";StoredWater=" << CurrentWater << ";";
